...
// Additional duplicate filtering by center distance
// Filters detections with same ID that are within max_distance pixels of each other
void FilterDuplicateDetectionsByCenter(zarray_t *detections, double max_distance = 100.0);
...
void RefineEdges(apriltag_detector_t *td, image_u8_t *im_orig,
  ...
  for (int edge = 0; edge < 4; edge++) {
    ...
    for (int s = 0; s < nsamples; s++) {
      ...
      // CRITICAL FIX: The coordinates x0, y0 come from quad->p which should be
      // in full resolution after AdjustPixelCenters(). However, if they're still
      // in decimated space, UnDistort() will produce wrong results. Since we
      // sample from the full-resolution image (im_orig), the coordinates should
      // be in full resolution. But if they're not, we need to scale them.
      // Actually, the real issue is that UnDistort() uses the camera matrix
      // which is calibrated for full resolution. If we pass decimated coordinates,
      // the output will be wrong. So we need to ensure coordinates are in
      // full resolution before calling UnDistort().
      // 
      // However, since we're sampling from the full-resolution image, and the
      // initial coordinates should be in full resolution, the coordinates should
      // already be correct. The bug must be elsewhere.
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (fabs(det) > 0.001) {
      ...
      // CRITICAL FIX: RefineEdges works in full resolution, but the coordinates
      // from UpdateFitQuads are in decimated space. After AdjustPixelCenters(),
      // they should be in full resolution, but RefineEdges recalculates them.
      // However, if the input coordinates to RefineEdges were wrong, the output
      // will also be wrong. Since we know the coordinates should be in full
      // resolution after AdjustPixelCenters(), and RefineEdges samples from
      // the full-resolution image, the output should already be correct.
      // But if it's not, we need to ensure they're scaled correctly.
      // Actually, the issue is that RefineEdges uses UnDistort which expects
      // full-resolution coordinates, so if we pass decimated coordinates, it
      // will produce wrong results. The fix is to ensure coordinates are
      // in full resolution BEFORE RefineEdges is called.
      quad->p[(i + 1) & 3][0] = px;
      quad->p[(i + 1) & 3][1] = py;
    } else {
...
static inline int detection_compare_margin(const void *_a, const void *_b) {
  apriltag_detection_t *a = *(apriltag_detection_t **)_a;
  apriltag_detection_t *b = *(apriltag_detection_t **)_b;
  if (a->decision_margin > b->decision_margin) return -1;
  if (a->decision_margin < b->decision_margin) return 1;
  return 0;
}
...
// Additional duplicate filtering by center distance
// Filters detections with same ID that are within max_distance pixels of each other
void FilterDuplicateDetectionsByCenter(zarray_t *detections, double max_distance) {
  // Sort by decision_margin (best first) so we keep the best detection
  zarray_sort(detections, detection_compare_margin);
  
  // Remove duplicates: for each detection, remove others with same ID within max_distance
  std::vector<bool> keep(zarray_size(detections), true);
  
  for (int i = 0; i < zarray_size(detections); i++) {
    if (!keep[i]) continue;
    
    apriltag_detection_t *det_i;
    zarray_get(detections, i, &det_i);
    
    // Calculate center of detection i
    double cx_i = 0, cy_i = 0;
    for (int j = 0; j < 4; j++) {
      cx_i += det_i->p[j][0];
      cy_i += det_i->p[j][1];
    }
    ...
    // Check all other detections
    for (int j = i + 1; j < zarray_size(detections); j++) {
      if (!keep[j]) continue;
      
      apriltag_detection_t *det_j;
      zarray_get(detections, j, &det_j);
      
      // Only filter if same ID
      if (det_i->id != det_j->id || det_i->family != det_j->family) {
        continue;
      }
      
      // Calculate center of detection j
      double cx_j = 0, cy_j = 0;
      for (int k = 0; k < 4; k++) {
        cx_j += det_j->p[k][0];
        cy_j += det_j->p[k][1];
      }
      ...
      // If within max_distance, remove the one with lower decision_margin
      if (dist < max_distance) {
        if (det_i->decision_margin >= det_j->decision_margin) {
          keep[j] = false;
        } else {
          keep[i] = false;
          break; // det_i is removed, no need to check more
        }
      }
    }
  }
  
  // Remove detections marked for removal (in reverse order to maintain indices)
  for (int i = zarray_size(detections) - 1; i >= 0; i--) {
    if (!keep[i]) {
      apriltag_detection_t *det;
      zarray_get(detections, i, &det);
      apriltag_detection_destroy(det);
      zarray_remove_index(detections, i, 1);
    }
  }
}
