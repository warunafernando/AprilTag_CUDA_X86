...
public:
  ...
  HostMemory(size_t size) {
    size_ = size;
    CHECK_CUDA(cudaMallocHost(reinterpret_cast<void **>(&data_), size_ * sizeof(T)));
  }
  HostMemory(const HostMemory &) = delete;
  HostMemory &operator=(const HostMemory &) = delete;

  virtual ~HostMemory() { CHECK_CUDA(cudaFreeHost(data_)); }
